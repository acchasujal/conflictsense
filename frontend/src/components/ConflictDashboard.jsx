...
function ExecutiveSummaryPanel({ conflicts, documentsCount }) {
  const crit = conflicts.filter((c) => c.severity === 'CRITICAL').length;
  const high = conflicts.filter((c) => c.severity === 'HIGH').length;
  const med  = conflicts.filter((c) => c.severity === 'MEDIUM').length;
    ...
    <div style={styles.panel}>
      ...
      {visibleConflicts.length > 0 && (
        <div style={styles.conflictsSection}>
          ...
          {phase !== 'done' && <SeveritySummary conflicts={visibleConflicts} />}

          {/* Executive Summary Panel */}
          {phase === 'done' && <ExecutiveSummaryPanel conflicts={visibleConflicts} documentsCount={documents.length} />}
